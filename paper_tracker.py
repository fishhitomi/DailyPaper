...
def summarize_papers_with_llm(papers):
    """调用大模型进行总结"""
    client = OpenAI(api_key=LLM_API_KEY, base_url="https://dashscope.aliyuncs.com/compatible-mode/v1")
...
3. 请严格按照以下 Markdown 格式输出中文:
**[试图解决的问题]**：(用一句话概括该研究针对的痛点或背景)
**[摘要翻译]**：(将摘要翻译成中文)
**[研究方法]**：(具体使用了什么数据、变量、模型和机制)
**[创新点]**：(有什么边际贡献，实现了什么指标提升，或提供了什么具体的建议)
...
"""
        ...
        response = client.chat.completions.create(
            model="deepseek-r1-distill-qwen-7b", messages=[{"role": "user", "content": prompt}]
        )
